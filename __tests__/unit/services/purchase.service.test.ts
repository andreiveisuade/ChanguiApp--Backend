...
describe('PurchaseService', () => {
  ...
  describe('getById', () => {
    ...
    it('lanza ApiError 404 si la compra no existe o no pertenece al usuario', async () => {
      ...
      await expect(purchaseService.getById(validUser.id, 'inexistente')).rejects.toMatchObject({
        status: 404,
      });
    });
    ...
    it('lanza ApiError 404 si purchase pertenece a otro user (filtro por user_id en repo)', async () => {
      ...
      await expect(purchaseService.getById(validUser.id, validPurchase.id)).rejects.toMatchObject({
        status: 404,
      });
    ...
    it('propaga errores de DB del repository', async () => {
      ...
      await expect(purchaseService.getById(validUser.id, validPurchase.id)).rejects.toThrow(
        'DB connection lost',
      );
    });
  });
